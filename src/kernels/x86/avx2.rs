...
// i8 max_abs: single-pass |v| (u8 0..128), one memory scan (32-wide).
#[target_feature(enable = "avx2")]
pub(crate) unsafe fn max_abs_i8(values: &[i8]) -> u8 {
    ...
    for i in 0..chunks {
        let v = unsafe { _mm256_loadu_si256(ptr.add(i * 32).cast::<__m256i>()) };
        let lo = unsafe { _mm256_cvtepi8_epi16(_mm256_castsi256_si128(v)) };
        let hi = unsafe { _mm256_cvtepi8_epi16(_mm256_extracti128_si256(v, 1)) };
        // abs in i16: max(lo, -lo)
        let alo = unsafe { _mm256_max_epi16(lo, _mm256_sub_epi16(_mm256_setzero_si256(), lo)) };
        let ahi = unsafe { _mm256_max_epi16(hi, _mm256_sub_epi16(_mm256_setzero_si256(), hi)) };
        acc = unsafe { _mm256_max_epi16(_mm256_max_epi16(acc, alo), ahi) };
    }
    let mut best: u16 = 0;
    if chunks > 0 {
        // horizontal max over 16 i16 lanes: fold halves via permute, then srli chain on low 128
        let folded = unsafe { _mm256_max_epi16(acc, _mm256_permute2x128_si256(acc, acc, 0x01)) };
        let lo128 = unsafe { _mm256_castsi256_si128(folded) };
        let m0 = unsafe { _mm_max_epi16(lo128, _mm_srli_si128(lo128, 8)) };
        let m1 = unsafe { _mm_max_epi16(m0, _mm_srli_si128(m0, 4)) };
        let m2 = unsafe { _mm_max_epi16(m1, _mm_srli_si128(m1, 2)) };
        let mut tmp = [0i16; 8];
        unsafe { _mm_storeu_si128(tmp.as_mut_ptr().cast::<__m128i>(), m2) };
    ...
    for i in 0..rem {
        let v = unsafe { *values.get_unchecked(chunks * 32 + i) };
...
crate::simd_variance_fused!(
    ...
    |p| unsafe { _mm256_loadu_ps(p) },
    _mm256_setzero_ps(),
    |acc: __m256, v: __m256, vm: __m256| unsafe {
        let d = _mm256_sub_ps(v, vm);
...
crate::simd_variance_fused!(
    ...
    |p| unsafe { _mm256_loadu_pd(p) },
    _mm256_setzero_pd(),
    |acc: __m256d, v: __m256d, vm: __m256d| unsafe {
        let d = _mm256_sub_pd(v, vm);
...
//
// 256-bit port of the SSE2 kernels (see `x86/sse2.rs` for the structure
...
// ≤ 1 ulp, f64 erfc ≤ 3 ulp; f32 widens to f64 and rounds once.
...
///
/// # Safety
/// Caller must ensure AVX2 is available (implied by the `target_feature`
/// gate on every caller).
#[cfg(feature = "alloc")]
#[inline]
#[target_feature(enable = "avx2")]
unsafe fn horner_r_256d(x2: __m256d) -> __m256d {
    let r = crate::kernels::erf::ERF_R;
    let mut acc = unsafe { _mm256_set1_pd(r[13]) };
    let mut i = 12;
    loop {
        acc = unsafe { _mm256_add_pd(_mm256_set1_pd(r[i]), _mm256_mul_pd(acc, x2)) };
        if i == 0 {
            break;
        }
        i -= 1;
...
#[inline]
#[target_feature(enable = "avx2")]
unsafe fn erfc1_pq_256d(s: __m256d) -> __m256d {
    let num = crate::kernels::erf::ERFC1_NUM;
    let den = crate::kernels::erf::ERFC1_DEN;
    let mut p = unsafe { _mm256_set1_pd(num[9]) };
    let mut q = unsafe { _mm256_set1_pd(den[9]) };
    let mut i = 8;
    loop {
        p = unsafe { _mm256_add_pd(_mm256_set1_pd(num[i]), _mm256_mul_pd(p, s)) };
        q = unsafe { _mm256_add_pd(_mm256_set1_pd(den[i]), _mm256_mul_pd(q, s)) };
        if i == 0 {
            break;
        }
        i -= 1;
...
unsafe fn erfc_small_256d(a: __m256d) -> __m256d {
    ...
    // low: 1 − (a + xr)
    let lo_val = unsafe { _mm256_sub_pd(_mm256_set1_pd(1.0), _mm256_add_pd(a, xr)) };
    // high: 0.5 − ((a − 0.5) + xr) — Sterbenz-exact split, no cancellation
    let hi_val = unsafe {
        _mm256_sub_pd(
            _mm256_set1_pd(0.5),
            _mm256_add_pd(_mm256_sub_pd(a, _mm256_set1_pd(0.5)), xr),
        )
    };
    let m_tiny =
    ...
    unsafe {
        _mm256_or_pd(
            ...
            _mm256_andnot_pd(
                m_tiny,
                _mm256_or_pd(
                    _mm256_and_pd(m_quarter, lo_val),
                    _mm256_andnot_pd(m_quarter, hi_val),
                ),
            ),
        )
    }
...
unsafe fn erfc_tail_raw_256d(a: __m256d) -> __m256d {
    ...
    let l = unsafe { tail_l_256d(a, u) };
    let e1 = unsafe {
        vexp_256d(_mm256_sub_pd(
            _mm256_sub_pd(_mm256_setzero_pd(), _mm256_mul_pd(z, z)),
            _mm256_set1_pd(0.5625),
        ))
    };
    let e2 = unsafe {
        vexp_256d(_mm256_add_pd(
            _mm256_mul_pd(_mm256_sub_pd(z, a), _mm256_add_pd(z, a)),
            l,
        ))
    };
    ...
    let c = unsafe {
        _mm256_or_pd(
            ...
            _mm256_andnot_pd(
                m_small,
                _mm256_or_pd(
                    _mm256_and_pd(m_mid, mid_val),
                    _mm256_andnot_pd(m_mid, tail_val),
                ),
            ),
        )
    };
...
unsafe fn verf_256d(v: __m256d) -> __m256d {
    ...
    let big_val = unsafe {
        _mm256_or_pd(
            _mm256_and_pd(neg, _mm256_sub_pd(_mm256_setzero_pd(), big_val)),
    ...
    let y = unsafe {
        _mm256_or_pd(
            _mm256_and_pd(m_small, small_val),
            _mm256_andnot_pd(m_small, big_val),
        )
    };
    unsafe { _mm256_or_pd(y, nan) }
}
...
crate::simd_map!(
    ...
    |p| unsafe { _mm256_loadu_pd(p) },
    |p, v| unsafe { _mm256_storeu_pd(p, v) },
    |v: __m256d| unsafe { verfc_256d(v) },
    |x: f64| crate::kernels::erf::erfc_f64(x)
);

// f32 erf/erfc: widen 8 floats to two f64 quads, run the f64 vector op,
...
crate::simd_map!(
    ...
    |v: __m256| unsafe {
        ...
        let rlo = _mm256_cvtpd_ps(verf_256d(lo));
        let rhi = _mm256_cvtpd_ps(verf_256d(hi));
        _mm256_insertf128_ps(_mm256_castps128_ps256(rlo), rhi, 1)
    },
...
crate::simd_map!(
    ...
    |v: __m256| unsafe {
        let lo = _mm256_cvtps_pd(_mm256_castps256_ps128(v));
        let hi = _mm256_cvtps_pd(_mm256_extractf128_ps(v, 1));
        let rlo = _mm256_cvtpd_ps(verfc_256d(lo));
        let rhi = _mm256_cvtpd_ps(verfc_256d(hi));
        _mm256_insertf128_ps(_mm256_castps128_ps256(rlo), rhi, 1)
    },
...
pub(crate) unsafe fn bf16_to_f32(input: &[u16], output: &mut [f32]) {
    ...
    unsafe {
        while i + 8 <= n {
            let v128 = _mm_loadu_si128(input.as_ptr().add(i).cast::<__m128i>());
            let zero = _mm_setzero_si128();
            let lo = _mm_unpacklo_epi16(v128, zero);
            let hi = _mm_unpackhi_epi16(v128, zero);
            let lo_shift = _mm_slli_epi32(lo, 16);
            let hi_shift = _mm_slli_epi32(hi, 16);
            let res = _mm256_set_m128i(hi_shift, lo_shift);
            _mm256_storeu_ps(output.as_mut_ptr().add(i), _mm256_castsi256_ps(res));
            i += 8;
        }
    }
    for j in i..n {
...
pub(crate) unsafe fn f32_to_bf16(input: &[f32], output: &mut [u16]) {
    ...
    let mut i = 0;
    unsafe {
        let c_one = _mm256_set1_epi32(1);
        let c_7fff = _mm256_set1_epi32(0x7FFF);
        let c_exp = _mm256_set1_epi32(0xFF);
        let c_mant = _mm256_set1_epi32(0x007F_FFFF);
        let c_40 = _mm256_set1_epi32(0x40);
        let zero = _mm256_setzero_si256();
        while i + 8 <= n {
            let v = _mm256_loadu_ps(input.as_ptr().add(i));
            let bits = _mm256_castps_si256(v);
            let lsb = _mm256_and_si256(_mm256_srli_epi32(bits, 16), c_one);
            let bias = _mm256_add_epi32(lsb, c_7fff);
            let added = _mm256_add_epi32(bits, bias);
            let rounded = _mm256_srli_epi32(added, 16);
            let exp = _mm256_and_si256(_mm256_srli_epi32(bits, 23), c_exp);
            let exp_eq = _mm256_cmpeq_epi32(exp, c_exp);
            let mant = _mm256_and_si256(bits, c_mant);
            let mant_zero = _mm256_cmpeq_epi32(mant, zero);
            let is_nan = _mm256_andnot_si256(mant_zero, exp_eq);
            let nan_bits = _mm256_or_si256(_mm256_srli_epi32(bits, 16), c_40);
            let res32 = _mm256_or_si256(
                _mm256_and_si256(is_nan, nan_bits),
                _mm256_andnot_si256(is_nan, rounded),
            );
            let mut tmp = [0u32; 8];
            _mm256_storeu_si256(tmp.as_mut_ptr().cast::<__m256i>(), res32);
            for k in 0..8 {
                output[i + k] = tmp[k] as u16;
            }
            i += 8;
        }
    }
...
pub(crate) unsafe fn dot_bf16(a: &[u16], b: &[u16]) -> f32 {
    ...
    unsafe {
        while i + 8 <= n {
            let va128 = _mm_loadu_si128(a.as_ptr().add(i).cast::<__m128i>());
            let vb128 = _mm_loadu_si128(b.as_ptr().add(i).cast::<__m128i>());
            let zero = _mm_setzero_si128();
            let va_lo = _mm_unpacklo_epi16(va128, zero);
            let va_hi = _mm_unpackhi_epi16(va128, zero);
            let vb_lo = _mm_unpacklo_epi16(vb128, zero);
            let vb_hi = _mm_unpackhi_epi16(vb128, zero);
            let fa = _mm256_castsi256_ps(_mm256_set_m128i(
                _mm_slli_epi32(va_hi, 16),
                _mm_slli_epi32(va_lo, 16),
            ));
            let fb = _mm256_castsi256_ps(_mm256_set_m128i(
